...
_(Paper prototypes showing early design concepts)_
...
- **Camera** (`CAMERA`): Required for scanning QR codes on electronic components during check-in/out operations.
- **Location** (`ACCESS_FINE_LOCATION`): Used to verify that users are at the Delta building before allowing item borrowing. This ensures items can only be checked out from the designated location.
- **Internet** (`INTERNET`): Required for Firebase Authentication, Firestore database operations, and fetching external API data (for example, the Quote of the Day).
- **Notifications** (`POST_NOTIFICATIONS`): Used to send return deadline reminders for borrowed items (7, 3, 1, and 0 days before due date).

### Data Handling & Storage

The app handles user data with the following approach:

- **Cloud Storage:** All user data, inventory items, and transaction history are stored in **Firebase Firestore** (cloud database). No local database is used.
- **Authentication Data:** User credentials are managed securely by **Firebase Authentication**. The app supports email/password and Google Sign-In authentication methods. All passwords are hashed and stored by Firebase.
- **User Profile Data:** Basic user information (name, email, borrowed items list, scan history) is stored in Firestore under the `users_real` collection.
- **Location Data:** Location information is used **only** at the time of scanning to verify the user's presence at the Delta building. Location data is **not stored** or sent to any server.
- **Theme Preferences:** User's dark/light mode preference is stored locally in **SharedPreferences** on the device.
- **Data Security:** Firebase provides built-in security with authentication tokens and Firestore security rules. All network communication uses HTTPS.
- **Data Retention:** User data persists in Firestore for the duration of the account's existence. Account deletion is not currently implemented in the app and account data is stored until project manager manual intervention.
